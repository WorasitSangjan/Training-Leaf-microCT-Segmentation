"""
Leaf CT Segmentation — DeepLabV3+ + EfficientNet-B4  [Phase 1]
===================================================
...
SLURM: torchrun --nproc_per_node=4 train_3_deeplab_efficientnet.py
"""
...
MODEL_DIR  = f"{OUTPUT_DIR}/models_DeepLab_EfficientNetB4"
LOG_DIR    = f"{OUTPUT_DIR}/logs_DeepLab_EfficientNetB4"
...
model = smp.DeepLabV3Plus(encoder_name="efficientnet-b4", encoder_weights="imagenet",
...
if is_main:
    print("=" * 70)
    print(f"[CONFIG] model=DeepLabV3+/EfficientNetB4 LR={LR} wd=5e-3 patch={PATCH_SIZE} stride={STRIDE} "
...
if is_main:
    ...
    model_eval = smp.DeepLabV3Plus(encoder_name="efficientnet-b4", encoder_weights=None, in_channels=1, classes=NUM_CLASSES)
