"""
Leaf CT Segmentation — EoMT + DINOv3 ViT-L/16
===============================================
SLURM batch script version. Run via submit_train11.sh
...
MODEL_DIR  = f"{OUTPUT_DIR}/models_EoMT_ViTL"
LOG_DIR    = f"{OUTPUT_DIR}/logs_EoMT_ViTL"
...
# ============================================================
# MODEL — EoMT with DINOv3 ViT-L/16
# ============================================================
class EoMT_ViTL(nn.Module):
    """
    Encoder-only Mask Transformer using DINOv3 ViT-L/16 backbone.
    ViT-L: hidden_size=1024, 24 layers (vs ViT-B: 768-dim, 12 layers)
    Based on: 'Your ViT is Secretly an Image Segmentation Model' (arxiv 2503.19108)
    Backbone: facebook/dinov3-vitl16-pretrain-lvd1689m (DINOv3, arxiv 2508.10104)
    """
...
if is_main:
    ...
    print("=" * 70)
    print(f"[CONFIG] LR={LR} wd=5e-3 patch={PATCH_SIZE} stride={STRIDE} "
          f"patience={PATIENCE} class_weights=[1.0,4.25,3.15,1.55,4.95] aug=phase1(flip+rotate) loss=CE "
          f"loss=CE+Jaccard elastic=True bg_only=True")
    print("=" * 70)
...
for epoch in range(start_epoch, NUM_EPOCHS):
    ...
    if is_main:
        ...
        # Update best_summary.txt whenever a new best is saved
        if saved_str:
