"""
Leaf CT Segmentation — FPN + MiT-B5
====================================
SLURM batch script version. Run via submit_train10.sh
...
MODEL_DIR  = f"{OUTPUT_DIR}/models_FPN_MitB5"
LOG_DIR    = f"{OUTPUT_DIR}/logs_FPN_MitB5"
...
if is_main:
    ...
    print(f"[CONFIG] LR={LR} wd=5e-3 patch={PATCH_SIZE} stride={STRIDE} "
          ...
          f"dropout=CoarseDropout(holes=1-8,16-32px,p=0.3) bg_only=True")
    print("=" * 70)
...
for epoch in range(start_epoch, NUM_EPOCHS):
    ...
    # Sync best_val_loss and epochs_no_improve from rank 0 to all ranks
...
if is_main:
    ...
    model_eval  = smp.FPN(encoder_name="mit_b5", encoder_weights=None,
