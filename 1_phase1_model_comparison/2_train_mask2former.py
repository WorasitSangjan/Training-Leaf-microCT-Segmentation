"""
Leaf CT Segmentation — Mask2Former + Swin-B  [Phase 1]
=======================================================
Query-based transformer without DINOv2 pretraining.
Uses HuggingFace transformers — model must be pre-cached:
...
SLURM: torchrun --nproc_per_node=4 train_7_mask2former.py
"""
...
os.environ['TRANSFORMERS_CACHE'] = '/pscratch/sd/w/worasit/torch_cache/huggingface'
...
MODEL_DIR  = f"{OUTPUT_DIR}/models_Mask2Former_SwinB"
LOG_DIR    = f"{OUTPUT_DIR}/logs_Mask2Former_SwinB"
...
PATCH_SIZE   = 320
STRIDE       = 160
BATCH_SIZE   = 16   # Mask2Former is heavier — reduce batch size
NUM_EPOCHS   = 100
LR           = 1e-4
VAL_FRAC     = 0.2
PATIENCE     = 15
...
# ============================================================
# MASK2FORMER WRAPPER
# Adapts HuggingFace Mask2Former to our dense prediction pipeline:
#   input : (B, 1, H, W)  — single channel, expanded to 3ch internally
#   output: (B, C, H, W)  — semantic logits via query aggregation
#
# Query aggregation: semantic_logits[b,c,h,w] = sum_q( p(c|q) * sigmoid(mask_q[h,w]) )
# where p(c|q) is the softmax class probability for query q.
# ============================================================
class Mask2FormerWrapper(nn.Module):
    ...
    def forward(self, x):
        # x: (B, 1, H, W) — expand grayscale to 3-channel RGB
        H, W   = x.shape[2], x.shape[3]
        x_rgb  = x.expand(-1, 3, -1, -1)
        out    = self.model(pixel_values=x_rgb)

        # class_queries_logits: (B, Q, num_classes+1)  — +1 is "no-object"
        # masks_queries_logits: (B, Q, H/4, W/4)
        class_logits = out.class_queries_logits[..., :self.num_classes]   # (B, Q, C)
        class_probs  = class_logits.softmax(-1)                           # (B, Q, C)

        masks = F.interpolate(out.masks_queries_logits,                   # (B, Q, H, W)
                              size=(H, W), mode='bilinear', align_corners=False)
        mask_probs = masks.sigmoid()                                       # (B, Q, H, W)

        # Aggregate: for each class sum query contributions weighted by class probability
...
class PatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            img_np  = img_p.squeeze(0).numpy(); mask_np = mask_p.numpy().astype(np.int32)
...
val_size = int(len(base_ds) * VAL_FRAC); trn_size = len(base_ds) - val_size
...
if is_main:
    print("=" * 70)
    print(f"[CONFIG] model=Mask2Former/SwinB LR={LR} wd=5e-3 patch={PATCH_SIZE} stride={STRIDE} "
          f"patience={PATIENCE} class_weights=[1.0,4.25,3.15,1.55,4.95] "
          f"aug=phase1(flip+rotate) loss=CE batch={BATCH_SIZE} bg_only=True")
    print("=" * 70)
