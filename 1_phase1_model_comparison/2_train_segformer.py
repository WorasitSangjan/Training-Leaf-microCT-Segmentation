"""
Leaf CT Segmentation — SegFormer-B4  [Phase 1]
===============================================
Full transformer: no CNN decoder (MLP decoder only).
Uses HuggingFace transformers — model must be pre-cached:
  python -c "from transformers import SegformerForSemanticSegmentation; \
             SegformerForSemanticSegmentation.from_pretrained('nvidia/segformer-b4-finetuned-ade-512-512', \
             num_labels=5, ignore_mismatched_sizes=True)"
SLURM: torchrun --nproc_per_node=4 train_6_segformer.py
"""
...
MODEL_DIR  = f"{OUTPUT_DIR}/models_SegFormer_B4"
LOG_DIR    = f"{OUTPUT_DIR}/logs_SegFormer_B4"
...
# ============================================================
# SEGFORMER WRAPPER
# Adapts HuggingFace SegFormer to our pipeline:
#   input : (B, 1, H, W)  — single channel grayscale
#   output: (B, C, H, W)  — full resolution logits
# ============================================================
class SegFormerWrapper(nn.Module):
    def __init__(self, num_classes, pretrained="nvidia/segformer-b4-finetuned-ade-512-512"):
        ...
        # Patch first patch embedding conv: 3-channel → 1-channel
        # Initialise with the mean of the 3-channel weights to preserve pretrained features
...
if is_main:
    print("=" * 70)
    print(f"[CONFIG] model=SegFormer-B4 LR={LR} wd=5e-3 patch={PATCH_SIZE} stride={STRIDE} "
