"""
Leaf CT Segmentation — Mask2Former + Swin-B  [Phase 2]
====================================================================
Training script for the Phase 2 Mask2Former candidate that produced the
best held-out mIoU among the fine-tuned Mask2Former runs.

Best-result recipe:
  - Backbone: Swin-B
  - Loss: 0.4 * weighted CE + 0.4 * per-class Tversky + 0.2 * Lovasz
  - Tversky alpha/beta: 0.7 / 0.3
  - Class weights: [1.00, 5.00, 4.00, 2.00, 4.60]
  - EMA decay: 0.999
  - Patch oversampling: classes 1, 2, and 4 at 4x
  - Augmentation: uint8 augment first, then normalize
  - Patch size/stride: 320 / 160

SLURM: torchrun --standalone --nproc_per_node=4 1_train_mask2former.py
"""
...
os.environ['TRANSFORMERS_OFFLINE'] = '1'      # V22/V19: blocks safetensors auto-conv lookup
...
import segmentation_models_pytorch as smp   # V22: Lovasz loss
...
MODEL_DIR  = f"{OUTPUT_DIR}/models_Mask2Former_SwinB_V20"
LOG_DIR    = f"{OUTPUT_DIR}/logs_Mask2Former_SwinB_V20"
...
# V19 settings (kept in V20)
EMA_DECAY    = 0.999     # EMA decay (~last 1000 steps)
CE_W         = 0.4       # was 1.0 in V17 (CE + Dice unweighted)
TVERSKY_W    = 0.4       # was 0.4 Dice in V19; same magnitude
LOVASZ_W     = 0.2       # V19 kept

# V20 NEW — Tversky parameters
TVERSKY_ALPHA = 0.7      # FP penalty weight   (Dice equivalent: 0.5)
TVERSKY_BETA  = 0.3      # FN penalty weight   (Dice equivalent: 0.5)

# ============================================================
# MASK2FORMER + SWIN-L WRAPPER
# ============================================================
class Mask2FormerWrapper(nn.Module):
    def __init__(self, num_classes,
        ...
        # use_safetensors=True is REQUIRED because:
        #   - torch < 2.6 refuses to load .bin via torch.load (CVE-2025-32434)
        #   - safetensors files bypass that check
        # Pre-download the safetensors on a NERSC login node BEFORE running:
        #   from huggingface_hub import snapshot_download
        #   snapshot_download(repo_id="facebook/mask2former-swin-base-IN21k-ade-semantic",
        #                     cache_dir="/pscratch/sd/w/worasit/.cache/huggingface/hub",
        #                     allow_patterns=["*.safetensors", "*.json", "*.txt"])
        # Once safetensors are in cache, from_pretrained finds them locally and
        # skips the HF Hub auto_conversion lookup that fails in offline mode.
        # revision="refs/pr/2" is REQUIRED because:
        #   - The main commit of this repo only has pytorch_model.bin (no safetensors)
        #   - PR #2 added the safetensors conversion
        #   - Without specifying this, transformers tries main snapshot, finds only
        #     .bin, falls back to auto_conversion (which fails offline).
        # The PR/2 snapshot has both config.json and model.safetensors.
    ...
    def forward(self, x):
        ...
        class_logits = out.class_queries_logits[..., :self.num_classes]
        class_probs  = class_logits.softmax(-1)

        masks      = F.interpolate(out.masks_queries_logits,
...
class LeafDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # V14: return RAW uint8 tensor. PatchDataset augments on uint8, then normalizes.
        img_t = torch.from_numpy(img).unsqueeze(0)   # uint8, shape (1, H, W)
...
class PatchDataset(Dataset):
    def __init__(self, base_dataset, patch_size=256, stride=128,
        ...
        # V17: KEEP V14's full aug stack (with CLAHE + RandomGamma on uint8).
        # Evidence: V16 (reduced aug, Swin-L) lost 0.060 on Wheat.
        # Lightly Train (different aug) lost 0.122 on Wheat.
        # Conclusion: full aug is needed for cross-species generalization,
        # independent of backbone size. Only change vs V14 is backbone (Swin-L -> Swin-B).
        self.aug = A.Compose([
    ...
    def __getitem__(self, idx):
        ...
        img, mask   = self.base[img_i]    # img is uint8 (1, H, W)
        ...
        if pad_h > 0 or pad_w > 0:
            img_p  = F.pad(img_p,  (0, pad_w, 0, pad_h), value=0)         # uint8 pad
        ...
        # V14: AUGMENT on uint8 (CLAHE/RandomGamma require uint8 input)
        if self.augment:
        ...
        # V14: Normalize AFTER augmentation, on the final uint8 patch.
        img_p_f = img_p.float()
...
# V13: V9 baseline + clean split ONLY. No BL x2 — EoMT V14/V15/V16 proved BL x2 doesn't help.
# Pine x3 retained (matches V9, equalizes per-species passes between BL and pine).
train_configs = bl_train + pine_train * 3   # ~45 + ~72 = ~117 configs (matches V9 ratio)
...
if is_main: print(f"Train patches: {len(train_patch_ds)} | Val patches: {len(val_patch_ds)}")
...
model = Mask2FormerWrapper(num_classes=NUM_CLASSES).to(device)

# V22 NEW class weights: Epi 4.15->5.0, Vas 3.10->4.0 (M2F bottleneck), Mes 1.5->2.0
class_weights = torch.tensor([1.00, 5.00, 4.00, 2.00, 4.60]).to(device)

# V22 NEW: Lovasz-Softmax loss instance (multiclass, expects logits)
loss_lovasz = smp.losses.LovaszLoss(mode="multiclass", from_logits=True)


# V20 loss: 0.4 * weighted_CE + 0.4 * per-class Tversky(0.7, 0.3) + 0.2 * Lovasz
def criterion(pred, mask):
    ...
    # --- 2. V20 NEW: per-class Tversky (replaces V19 per-class Dice) ---
    ...
    # --- 3. Lovasz on spatial logits (kept from V19) ---
...
# ============================================================
# V22 NEW — EMA helper (mirrors EoMT V19/V20/V21)
# ============================================================
class ModelEMA:
    """
    Maintains an Exponential Moving Average of model parameters.
    Stored on the same device as the model (sync'd across DDP ranks).
    """
...
def _swap_in_weights(model_module, new_sd):
    """Swap model weights for `new_sd`; return prior state for restore."""
...
if is_main:
    ...
    print(f"Class weights : {class_weights.tolist()}  (V19/V20: Epi 5, Vas 4, Mes 2)")
...
if is_main:
    print("=" * 70)
    print(f"[CONFIG V20] model=Mask2Former/SwinB LR={LR} wd=5e-3 patch={PATCH_SIZE} stride={STRIDE} "
          f"patience={PATIENCE} class_weights=[1.00,5.00,4.00,2.00,4.60] (V19 kept) "
          f"oversample=Epi+Vas+Air(4x) "
          f"loss={CE_W}*CE+{TVERSKY_W}*Tversky(a={TVERSKY_ALPHA},b={TVERSKY_BETA})+{LOVASZ_W}*Lovasz (V20 NEW Tversky) "
          f"EMA decay={EMA_DECAY} (V19 kept) "
          f"scheduler=ReduceLROnPlateau "
          f"aug=flip+rotate50+elastic+noise+bc+RandomGamma+CLAHE+blur+coarsedropout (V17 unchanged) "
          f"PIPELINE:uint8-augment-then-normalize "
          f"batch={BATCH_SIZE} "
          f"train={len(train_configs)}configs({len(bl_train)}BL+{len(pine_train)}pinex3) "
          f"val={len(val_configs)}configs (V19 + Dice->Tversky single-variable swap)")
    print("=" * 70)
...
if checkpoint_path:
    ...
    # V22: prefer live weights for resume; fall back to model_state_dict
...
for epoch in range(start_epoch, NUM_EPOCHS):
    ...
    for x, y in tqdm(train_loader, desc=f"Ep {epoch+1}/{NUM_EPOCHS} [Train]", leave=False, disable=not is_main):
        ...
        # V22 NEW: update EMA after every optimizer step
    ...
    # V22 NEW: validate using EMA weights, then restore live weights
    backup_sd = _swap_in_weights(model.module, ema.state_dict())

    model.eval(); val_loss = 0.0; vb = 0
    with torch.no_grad():
        for x, y in tqdm(val_loader, desc=f"Ep {epoch+1}/{NUM_EPOCHS} [Val-EMA]", leave=False, disable=not is_main):
            x, y = x.to(device, non_blocking=True), y.to(device, non_blocking=True)
            with autocast('cuda', dtype=torch.bfloat16):
    ...
    model.module.load_state_dict(backup_sd)
    del backup_sd
    ...
    if is_main:
        torch.save({'epoch': epoch, 'model_state_dict': get_model_state_dict(model),
                    'ema_state_dict': ema.state_dict(),               # V22 NEW
        ...
        if avg_val < best_val_loss:
            ...
            # V22 NEW: best ckpt's `model_state_dict` IS the EMA weights so
            # existing 2_evaluate.py picks them up without changes.
            torch.save({'epoch': epoch+1,
                        'model_state_dict': ema.state_dict(),         # V22 NEW (EMA)
...
# ============================================================
if is_main:
    print("\n" + "=" * 70 + "\nEvaluating best model (EMA weights)...")
    CLASS_NAMES = ["Background", "Epidermis", "Vascular_Region", "Mesophyll", "Air_Space"]

    ckpt       = torch.load(BEST_MODEL_PATH, map_location=device)
    model_eval = Mask2FormerWrapper(num_classes=NUM_CLASSES)
