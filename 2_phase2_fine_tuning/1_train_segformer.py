"""
Leaf CT Segmentation — SegFormer-B4  [Phase 2]
====================================================================
Training script for the Phase 2 SegFormer candidate that produced the
best held-out mIoU among the fine-tuned SegFormer runs.

Best-result recipe:
  - Backbone: SegFormer-B4 with 1-to-3 channel adapter
  - Loss: 0.4 * weighted CE + 0.4 * per-class Tversky + 0.2 * Lovasz
  - Tversky alpha/beta: 0.7 / 0.3
  - Class weights: [1.00, 5.00, 3.50, 2.00, 4.60]
  - EMA decay: 0.999
  - Patch oversampling: classes 1, 2, and 4 at 4x
  - Augmentation: uint8 augment first, then normalize
  - Patch size/stride: 320 / 160

SLURM: torchrun --standalone --nproc_per_node=4 1_train_segformer.py
"""
...
MODEL_DIR  = f"{OUTPUT_DIR}/models_SegFormer_B4_V4"
LOG_DIR    = f"{OUTPUT_DIR}/logs_SegFormer_B4_V4"
if is_main:
...
# V3 settings (kept in V4)
EMA_DECAY    = 0.999
CE_W         = 0.4
TVERSKY_W    = 0.4       # was 0.4 Dice in V3; same magnitude
LOVASZ_W     = 0.2

# V4 NEW — Tversky parameters
...
class LeafDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # V3: return RAW uint8 (PatchDataset augments uint8 then normalizes)
...
# ============================================================
# SegFormer-B4 WRAPPER (matches Phase 1 clean / segformer_v2 checkpoint)
# ============================================================
...
# V3 SegFormer-tuned class weights (mild Vas boost only):
class_weights = torch.tensor([1.00, 5.00, 3.50, 2.00, 4.60]).to(device)
loss_lovasz   = smp.losses.LovaszLoss(mode="multiclass", from_logits=True)


def criterion(pred, mask):
    """V4: 0.4 * weighted_CE + 0.4 * per-class Tversky(0.7, 0.3) + 0.2 * Lovasz"""
    b, c, h, w = pred.shape
    ...
    # --- 2. V4 NEW: per-class Tversky (replaces V3 per-class Dice) ---
    ...
    # --- 3. Lovasz on spatial logits (kept from V3) ---
...
if is_main:
    ...
    print(f"Class weights : {class_weights.tolist()}  (V3 kept: Epi 5, Vas 3.5 mild, Mes 2)")
    print(f"Loss weights  : CE={CE_W}  Tversky={TVERSKY_W} (alpha={TVERSKY_ALPHA}, beta={TVERSKY_BETA})  Lovasz={LOVASZ_W}")
    print("=" * 70)
    print(f"[CONFIG SegFormer V4] LR={LR} wd=5e-3 patch={PATCH_SIZE} stride={STRIDE} "
          f"patience={PATIENCE} class_weights=[1.00,5.00,3.50,2.00,4.60] (V3 kept) "
          f"oversample=Epi+Vas+Air(4x) "
          f"loss={CE_W}*CE+{TVERSKY_W}*Tversky(a={TVERSKY_ALPHA},b={TVERSKY_BETA})+{LOVASZ_W}*Lovasz (V4 NEW Tversky) "
          f"EMA={EMA_DECAY} (V3 kept) "
          ...
          f"train={len(train_configs)} val={len(val_configs)} (V3 + Dice->Tversky single-variable swap)")
    print("=" * 70)
...
if is_main:
    print("=" * 70)
    print(f"Training complete! SegFormer V4 best val loss (EMA): {best_val_loss:.4f}")
...
if is_main:
    ...
    model_eval = SegFormerWrapper(num_classes=NUM_CLASSES)
    ...
    print(f"\nSegFormer V4 Val Mean IoU (EMA): {np.mean(valid_iou):.4f}")
