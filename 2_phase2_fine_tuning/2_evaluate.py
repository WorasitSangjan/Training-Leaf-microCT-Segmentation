...
Usage:
  ...
  python 2_evaluate.py \
    --model eomt_vitl \
    --checkpoint /pscratch/sd/w/worasit/outputs/models_EoMT_ViTL_V17/best_model.pth \
...
class EoMT(nn.Module):
    """
    End-to-end Mask Transformer using DINOv3 backbone.
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate Phase 2 leaf segmentation checkpoints",
    ...
    parser.add_argument("--stride",      type=int, default=STRIDE,
                        help="Inference stride (default: 80, denser than training). "
    ...
    if args.per_image and per_img_tp:
        ...
        for gi in sorted(per_img_tp.keys()):
            ...
            for c in range(n):
                ...
                row[f'Precision_{cname}'] = round(p[c].item(),    4)
                row[f'Recall_{cname}']    = round(r[c].item(),    4)
